...
%save('sol_ort_original_inner','Ks','TK_end','T_end','X','Y','ell_inner','sol')
...
points = 'full'; % 'full' to use full Cheby's mesh; 'inner' (MS1) to use only inner domain points; 'selection' (MS2) to a especial selection of points
chosenNorm = 2; % 2, 'inf'
...
while (i <= kmax && flag == 0)
    ...
    for jj = 1:2
        ...
        subplot(2,5,ii+4)
        surf(X,Y,reshape(K,n+1,n+1)), zlim([0, 0.3])
        axis square
        title(['approx., err = ' num2str(err) ', int. err = ' num2str(err_int) ''])
        subplot(2,5,ii+1)
        surf(X,Y,reshape(Ke,n+1,n+1)), zlim([0, 0.3])
        axis square
...
for jj = 1:2
    ...
    subplot(2,4,ii+2)
    contour(X,Y,reshape(Ke,n+1,n+1),10);
    axis square
    title('exact (contour)')
    subplot(2,4,ii+3)
    contour(X,Y,reshape(K,n+1,n+1),10);
    axis square
...
% ht = findall(gcf,'type','axes'); set(ht,'fontsize',14);
% 
% export_fig cond_exact_n15_exp.eps -transparent
