...
for ip = 1:length(chosen)
    ...
    for in = 1:length(NL)
        ...
        %disp(['DETAILS: Approach 1 ("' points '" case), with ' num2str(100*delta) '% of noise, ' num2str(over) ' over (' num2str(over*ell*N) ' information)'])
        ...
        % 2) 'inner': select only points inside a given inner rectangular domain (defined in 'data.m')
        % 3) 'selection': special selection of points (only as example).
