...
N = 180; % number of equally spaced intervals on time discretization
tf = 60; % final time
...
delta = 0.0; % noise level (this case has no extra noise introduction)
...
% saveFileName = sprintf('EXPrep10k%dt%dN%dnorm2delta0reg1_Vrandn.mat',K0,final_t,N); % string to be used as name for the .mat file containing the results of this code

%points = 'selection'; % 'full' to use full Cheby's mesh; 'inner' to use only inner domain points; 'closest' to closest points; 'selection' to a especial selection of points
...
[Tq, tq0, ~] = Filtro(34,57,60); % filter raw data
...
if N == 60 && final_t == 30 % to correct wrong choices of time steps for this setting
    delta_nd = 1.5;
    tt0 = tq0(1:delta_nd:91);
    U = Tq(:,2:delta_nd:91);
end
...
% % SAVE DATA
% save(saveFileName,'K_LMM','K_TRR','K_TRR_ul','K_initial','Ke','TK_LMM',...
%     'TK_TRR','TK_TRR_ul','U','Up','X','Y','iter_LMM','iter_TRR',...
%     'iter_TRR_ul','parameters','t0','xs','ys');
